...
{
	if (	(fwd.GetNumRows() != 4) || (fwd.GetNumRows() != 4) ||
		(fwd.GetNumRows() != 4) || (fwd.GetNumRows() != 4))
		throw std::invalid_argument("inputs are not 4x4;");
				...
				const qbVector<double> &scale)
{
	...
	rotationMatrixZ.SetElement(1, 2, -sin(rotation.GetElement(0)));
	rotationMatrixZ.SetElement(2, 1, sin(rotation.GetElement(0)));
	rotationMatrixZ.SetElement(2, 2, cos(rotation.GetElement(0)));
